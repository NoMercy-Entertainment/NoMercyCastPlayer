...
<template>
  <article ref="el" class="music-home-card" data-focusable tabindex="0" role="button">
    <div class="cover">
      <img
        v-if="cover"
        :src="cover"
    ...
    <div class="meta">
      <p v-if="props.data.type" class="kind">{{ props.data.type }}</p>
      <p class="name">{{ props.data.name }}</p>
      <p v-if="props.data.artist" class="artist">{{ props.data.artist }}</p>
    </div>
  </article>
</template>
