...
useFocusEntry({
  ...
  onAction: () => {
    ...
    if (!e) return
    if (playbackStore.music.playing.value) e.pause()
    else e.play()
  },
})
useFocusEntry({
...
<template>
  <div class="controls">
    ...
    <div ref="containerEl" class="buttons">
      <button ref="prevEl" data-focusable tabindex="0" class="ctrl-btn">⏮</button>
      <button ref="playEl" data-focusable tabindex="0" class="ctrl-btn ctrl-play">
        {{ playing ? '⏸' : '▶' }}
      </button>
      <button ref="nextEl" data-focusable tabindex="0" class="ctrl-btn">⏭</button>
    </div>
  </div>
</template>
