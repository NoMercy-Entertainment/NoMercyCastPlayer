...
import { mountEpisodeScreen, type EpisodeItem } from './pluginPanels/EpisodeScreen'
import { mountLanguageScreen, type TrackItem } from './pluginPanels/LanguageScreen'
...
export class TVOverlayPlugin {
  ...
  attach(): void {
    if (!this.opts.skipPreScreen) this.showPreScreen()
  ...
  private showLanguageScreen(): void {
    ...
    this.unmount = mountLanguageScreen({
      ...
      onPickSubtitle: (id) => {
        if (id === 'off') this.player.setSubtitleTrack?.('')
  ...
  showSeekContainer(): void {
    const host = this.overlayHost()
    if (!host) return
    if (this.currentPanel === 'seek') return
    this.closeAllPanels()
  ...
  private maybeShowPreScreen(): void {
    if (this.currentPanel === null) this.showPreScreen()
  }

  private handleBack(): void {
    if (this.currentPanel === 'episode' || this.currentPanel === 'language') this.showPreScreen()
    else if (this.currentPanel === 'seek') this.closeAllPanels()
    else if (this.currentPanel === 'pre') this.player.emit?.('back')
    else {
      this.player.pause()
      this.showPreScreen()
    }
